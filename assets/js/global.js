...
var BOOKWORMJS = function () {
  ...
  */
  ...
  var mainNav = function(button) {	
		if ( button.hasClass('sqx') ) {
			$(button).removeClass('sqx');
  ...
  */
  this.init = function () { 
  	...
  	/**
    *  Establish and monitor viewport as either mobile or desktop
    */
      ...
      if (direction === "down") {
        $("header").addClass('hdr-sdw');       
      } else {
        $("header").removeClass('hdr-sdw');
      }
    }, { offset: 0, triggerOnce: false });
  	
  	$(".menu-item-has-children, .menu-item-has-children").on( "click", function() {
			var $thisItem = $(this);
			$thisItem.children('li').slideDown.slideDown( "slow", function() {
				// Animation complete.
			});
      return false;
		});
		...
		*/
		...
		console.log('Bookworm theme global.js');
